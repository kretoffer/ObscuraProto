...
bool Crypto::verify(const Signature& signature, const byte_vector& message, const PublicKey& public_key) {
    ...
    return crypto_sign_verify_detached(signature.data.data(), message.data(), message.size(), public_key.data.data()) == 0;
}

Crypto::SessionKeys Crypto::client_compute_session_keys(const KeyPair& client_kx_kp, const PublicKey& server_eph_pk) {
...
Crypto::SessionKeys Crypto::server_compute_session_keys(const KeyPair& server_kx_kp, const PublicKey& client_eph_pk) {
...
EncryptedPacket Crypto::encrypt(const Payload& payload, uint64_t counter, const byte_vector& key) {
    ...
    crypto_aead_chacha20poly1305_ietf_encrypt(
        packet.data() + nonce.size() + sizeof(be_counter), &ciphertext_len,
        plaintext.data(), plaintext.size(),
        reinterpret_cast<unsigned char*>(&be_counter), sizeof(be_counter),
    ...
    std::copy(reinterpret_cast<uint8_t*>(&be_counter), reinterpret_cast<uint8_t*>(&be_counter) + sizeof(be_counter), packet.begin() + nonce.size());
...
Crypto::DecryptedResult Crypto::decrypt(const EncryptedPacket& packet, const byte_vector& key) {
    ...
    if (crypto_aead_chacha20poly1305_ietf_decrypt(
            decrypted_payload_data.data(), &decrypted_len,
            nullptr, // nsec is not used
            ciphertext_with_tag, ciphertext_with_tag_len,
            reinterpret_cast<const unsigned char*>(&be_counter), sizeof(be_counter),
            nonce.data(),
            key.data()
        ) != 0) {
