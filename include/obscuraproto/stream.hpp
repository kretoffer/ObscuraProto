...
namespace ObscuraProto {
    ...
    public:
        ...
        Stream(uint32_t stream_id, SendFn send_fn)
            : stream_id_(stream_id), send_fn_(std::move(send_fn)) {}
        ...
        void set_data_handler(DataHandler handler) { on_data_ = std::move(handler); }
        void set_end_handler(EndHandler handler) { on_end_ = std::move(handler); }
        void set_cancel_handler(CancelHandler handler) { on_cancel_ = std::move(handler); }

        void dispatch_data(byte_vector data) { if (on_data_) on_data_(std::move(data)); }
        void dispatch_end() { if (on_end_) on_end_(); }
        void dispatch_cancel() { if (on_cancel_) on_cancel_(); }
