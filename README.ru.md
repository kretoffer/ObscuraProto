...
### 7.1. Жизненный цикл потока и структура полезной нагрузки
...
*   `sequence_number`: `uint64_t`, который указывает на порядок чанков данных внутри потока.

Полезная нагрузка для сообщения `STREAM_DATA` до шифрования выглядит так:
`[Код операции (2)] + [stream_id (4)] + [sequence_number (8)] + [часть_данных (N)]`

### 7.3. Пример использования API для стриминга
