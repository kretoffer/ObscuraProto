...
- **`Payload`**: Ключевой класс, представляющий "полезную нагрузку" — команды и данные в незашифрованном виде.
    ...
    - **`PayloadBuilder`**: Вспомогательный класс для удобного конструирования объектов `Payload`. Он предоставляет методы, такие как `add_param(std::string)`, `add_param(uint32_t)`, `add_param(byte_vector)`, которые используют сериализацию с префиксом длины (перед каждым параметром записывается его длина), что позволяет легко разделять их при чтении.
    - **`PayloadReader`**: Вспомогательный класс, который помогает удобно извлекать параметры по одному из полученного `Payload`. Он предоставляет методы, такие как `read_param_string()`, `read_param_u32()`, `read_param_bytes()`.
...
**Назначение:** `Payload` — это "конверт", в который вы кладёте команду (`op_code`) и её данные (`parameters`), прежде чем запечатать его (зашифровать). `PayloadBuilder` и `PayloadReader` упрощают процесс подготовки и интерпретации этого конверта.
...
- **`Session`**: Главный класс, управляющий состоянием соединения.
    ...
    - **`set_client_identity_key(...)`**: **Для клиента.** Устанавливает пару ключей Ed25519 для аутентификации клиента во время рукопожатия.
    - **`has_peer_identity()`**: Проверяет, предоставил ли подключающийся клиент и была ли проверена его идентификация.
    - **`get_peer_identity()`**: Возвращает проверенный публичный ключ Ed25519 подключающегося клиента, если он доступен.

### Аутентификация клиента (Client Identity)

Начиная с версии 1.0, ObscuraProto поддерживает **опциональную аутентификацию клиента**. Клиент может включить свой публичный ключ Ed25519 и подпись своего эфемерного ключа X25519 в сообщение `ClientHello`. Сервер проверяет подпись и, если она верна, связывает соединение с этим публичным ключом.

Сессии без идентификации считаются **анонимными** и направляются в отдельный набор обработчиков, что позволяет приложениям реализовывать регистрацию или вход в систему без аутентификации на уровне протокола.
